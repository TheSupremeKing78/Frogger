	...
	public void paintComponent(Graphics g) {
		...
		if (inf) {
			for (Element river : rivers) {
				g.drawImage(river.img, pixelByCase * river.absc, pixelByCase * (height - river.ord - 1), this);
			}
			for (Element line : lines) {
				g.drawImage(line.img, pixelByCase * line.absc, pixelByCase * (height - line.ord - 1), this);
			}
		}
		for (Element element : elementsToDisplay) {
			g.drawImage(element.img, pixelByCase * element.absc, pixelByCase * (height - 1 - element.ord ), this);
		}
		/*else {
			for (Element river : rivers) {
				e2 = river;
